...
# Phase 16A: TikTok official publishing foundation

TikTok becomes a real publishable provider alongside Instagram, using only TikTok's official Login Kit OAuth and Content Posting API (Direct Post) — no browser automation, private APIs, or CAPTCHA bypass anywhere in the flow. Login Kit requests the minimal `video.publish` scope; access and rotating refresh tokens reuse Instagram's AES-GCM credential encryption verbatim. Every Direct Post queries Creator Info immediately before initializing and again immediately before uploading, so privacy/comment/duet/stitch choices are always validated against the creator's current live options rather than a stale cache, while the chosen settings are still frozen onto the schedule/publication snapshot at creation time. FILE_UPLOAD (not PULL_FROM_URL) streams private object-storage media in bounded chunks from a single shared chunk-planning function, so the `chunk_size`/`total_chunk_count` declared to TikTok can never diverge from what is actually streamed. `publish_id` is persisted durably the instant TikTok confirms it, before any upload byte is sent, so a crash or restart always resumes from the stored id instead of re-initializing and risking a duplicate public post; a genuinely ambiguous outcome (e.g. a network failure during init or upload) is recorded as `PROVIDER_OUTCOME_UNKNOWN` and requires manual reconciliation rather than being retried automatically. The Worker only drives a provider-agnostic `/tiktok/drive` polling endpoint — it never sees a TikTok token, client secret, or upload URL; all TikTok HTTP calls happen backend-side. Unaudited clients remain restricted to `SELF_ONLY` per TikTok's current audit policy. `AUTO_SCHEDULE` is not enabled for TikTok (Robots' autonomous-provider gate stays TEST-only); TikTok analytics, Draft Upload mode, and photo posting are explicitly out of scope for this phase.
...
- Historical single-output runs remain valid without backfill. Cross-output campaign planning, adaptive cadence, multi-account publishing, and performance-driven selection remain outside this phase.

### Phase 17E — Campaign coherence & cross-output content planning (implemented)

- `CampaignContentPlan`/`CampaignContentPlanItem` add advisory, revisioned cross-output coordination on top of a multi-output `RobotRun` — never touching V3 candidate scores/ranks, `HighlightSelection` membership/order, `RobotRunOutput` membership/`selectionOrder`, clip boundaries, or `ExperimentAssignment`.
- Four `CampaignPlanningPolicy` values on `Robot`/frozen onto `RobotRun`: `NO_CAMPAIGN_PLAN` (default, byte-identical to Phase 17D), `DETERMINISTIC_PLAN` (`DETERMINISTIC_CAMPAIGN_V1`, synchronous, zero external dependency, auto-applies), `AI_PLAN_FOR_REVIEW`, and `AI_PLAN_AND_APPLY` (both `CAMPAIGN_PLAN_V1`, dispatched as a `GENERATE_CAMPAIGN_PLAN` job through the same Worker provider-abstraction and AI kill switch as `GENERATE_SOCIAL_COPY`).
- A plan's sequence always mirrors the frozen `RobotRunOutput.selectionOrder`; the AI is given a fixed output set and instructed never to add, remove, substitute, or reorder outputs, and the backend performs the only authoritative validation (exact output-id match, controlled `CampaignPlanRole` enum, bounded field lengths, deterministic lexical near-duplicate hook rejection) — nothing is persisted unless the whole structured response validates.
- Regeneration creates a new immutable revision rather than mutating history; exactly one revision is ever `is_current` per run (`campaign_content_plans_one_current_per_run`), enforced at the database level.
- Applying a plan is advisory only: it never approves a `ContentSuggestion`, a Robot publishing approval, or a `PublishSchedule`. When an output's AI copy is generated, it consumes the applied plan item's guidance through a new `SOCIAL_COPY_V3_CAMPAIGN` prompt version (historical `SOCIAL_COPY_V1`/`V2` prompts and fingerprints are untouched), and `PublicationAttribution` freezes the campaign plan/revision/item only when that guidance was actually consumed — never merely because a plan existed for the run.
- Campaign planning never reads `PublicationMetrics`, `PerformanceInsights`, or Experiment outcomes; Persona/experiment-treatment context is the same frozen run-creation snapshot Phase 12B/14A already established, never a fresh live lookup.
