...
# Phase 14C decision guardrails

`EXPERIMENT_GUARDRAILS_V1` consumes the existing Phase 14B analysis without recomputing Welch statistics. Statistical evidence, deterministic review readiness, and an operator's decision are separate. A DRAFT experiment requires a positive `minimumPracticalEffect` in absolute units of its primary metric; activation freezes it. Migrated active experiments remain valid with a null threshold, reported as `NOT_CONFIGURED`.

The ordered checks cover frozen definition, provider consistency, observed sample, maturity, assignment-outcome coverage (default 60% per arm), attrition gap (warning at 20 percentage points), protocol deviation rate (warning at 20%), analysis status, and practical-effect context. A blocked check yields `NOT_READY`; otherwise `READY_FOR_REVIEW`, even if the CI includes zero or the effect is below the practical threshold. A meeting threshold does not imply statistical certainty; an interval excluding zero does not imply practical importance. The direction and 95% CI relationship to `+/-threshold` are descriptive only. Neither status selects a variant. No LLM participates.

`experiment_decisions` is an append-only application-level audit history. POST accepts only decision, selected variant key, population, rationale, and UUID idempotency key. The server obtains membership/user/time, computes current readiness, freezes the primary metric/window, threshold, status, sample sizes, mean difference, CI, p-value, effect size, readiness, and SHA-256 over a canonical UTF-8 sequence with explicit null markers. Conflicting reuse of a key returns 409; new keys retain intentional later decisions. History is bounded to the newest 100. DRAFT decisions are rejected; ACTIVE, PAUSED, and COMPLETED allow all types; CANCELLED excludes variant selection. Decisions do not pause/complete or update operational entities. Further data or configuration changes do not rewrite audit rows. Phase 15 traffic allocation and optimization remain out of scope.

## Phase 15A decision applications

`DecisionApplicationService` is a control-plane boundary separate from decision recording. V1 supports only PERSONA experiments, SELECT_VARIANT_A/B, and one explicit Robot already linked to that Experiment. Variant lookup uses its stable A/B key and frozen Persona ID; operational eligibility uses the current live Persona status, so archived or missing Personas block application. Preview performs no writes and returns deterministic blocking reasons, warnings, one `personaId` diff, the frozen decision evidence, and a SHA-256 `DECISION_APPLICATION_V1` precondition fingerprint.

Apply locks the workspace Robot pessimistically, recomputes preview, rejects stale fingerprints, validates independent ACTIVE/NOT_READY/older-decision acknowledgements, and mutates only `Robot.persona`. Experiment linkage and lifecycle remain intact. The V27 `decision_applications` table freezes before/after names and IDs, fingerprints, operator/time, confirmations, and timestamps. Retries are unique by workspace/decision/Robot/idempotency key. No-op applications avoid a Robot update but retain an audit row. Rollback is separately previewed and fingerprinted; it creates another immutable row and is blocked when Robot state diverged or the prior Persona is missing/archived. The kill switch disables apply/rollback only. There is no LLM, automatic application, traffic allocation, bulk rollout, force rollback, or autonomous optimization.

## Phase 16A TikTok publishing

TikTok stays inside `Publication -> PUBLISH_MEDIA -> backend provider`. Login Kit requests only the minimal `video.publish` scope (no `user.info.basic`, since Creator Info already supplies the identity fields the app uses, and requesting an unused scope only invites app-review friction); state is hashed, short-lived, workspace/user-bound, and single-use. Access and rotating refresh tokens use AES-GCM. Each Direct Post queries Creator Info, validates media and frozen settings, initializes `/v2/post/publish/video/init/`, persists `publish_id`, uploads sequential `FILE_UPLOAD` chunks from a single shared chunk plan (so the declared `chunk_size`/`total_chunk_count` can never diverge from what is actually streamed), and polls `/v2/post/publish/status/fetch/`. Capability URLs are not exposed or logged. Ambiguous post-init outcomes become `PROVIDER_OUTCOME_UNKNOWN`; unverified clients remain `SELF_ONLY`. Analytics and autonomous TikTok scheduling are outside Phase 16A.

## Phase 17A Semantic Highlighting V2

`HighlightAnalysis`/`HighlightCandidate` already supported multiple analyses per asset and a free-form `analyzerType`/`analyzerVersion` pair (no schema change needed for a third analyzer family); the repository's actual pre-17A state is `DETERMINISTIC_V1` (Phase 7A, a fixed-position heuristic, no transcript) and `TRANSCRIPT_SEMANTIC_V1` (Phase 7B2's real name in this codebase — an Ollama-backed local-LLM analyzer behind `HighlightAnalyzer`, deliberately excluded from Robot automation because the Ollama runtime is optional and was observed disabled in this project's own Docker/Worker stack). `DETERMINISTIC_V2` is a third, independent analyzer: transcript-driven like `TRANSCRIPT_SEMANTIC_V1`, but deterministic and LLM-free like `DETERMINISTIC_V1`, so it carries neither's weakness. It reuses the exact same `ANALYZE_HIGHLIGHTS` job, Worker-claim, authorize/complete contract as the other two (`AnalyzeHighlightsExecutor` dispatches to a `Map<String, HighlightAnalyzer>` keyed by wire type) — the Worker receives transcript segments plus the effective V2 config in the authorization response and returns scored candidates; validation, transcript-boundary grounding, stable re-ranking, and persistence stay backend-side exactly as for the other two analyzers.

V2's algorithm (`DeterministicMultimodalHighlightAnalyzer`, Worker-side, pure Java, zero network calls): Unicode punctuation heuristics (`.!?…‽`) build two boundary sets — a strict "sentence" set (real idea starts/ends) and a broader "all" set (sentence boundaries plus every transcript segment's own start/end, since ASR segment cuts are never mid-word). Candidate starts are a deterministically-downsampled, timeline-spanning subset of the broad boundary set (bounded by `v2-max-candidate-starts`), each producing at most a couple of duration-bounded windows (bounded by `v2-max-candidate-windows`) — so a 2-3 hour transcript (bounded again by `v2-max-segments-considered`, deterministic even-stride downsampling above that) still produces a small, constant-bounded candidate set, never an O(n^2) blow-up. Each window is scored on six weighted, independently-persisted 0-1 components (`hookScore`, `completenessScore`, `informationDensityScore`, `speechDensityScore` — a preferred-band function of speech-to-window-duration ratio, not "more speech is always better" — `boundaryScore`, `coverageScore`), gated by a minimum-coverage eligibility threshold, then run through deterministic NMS: sort by base score descending with a fully deterministic tie-break (start ms, end ms, a content-hash key), greedily accept, suppressing any candidate whose temporal IoU or token-Jaccard text similarity against an already-accepted candidate exceeds its configured threshold. `sceneScore`/`audioBoundaryScore` columns exist but are always null in 17A — no scene-cut or `silencedetect` extraction pipeline was built (out of scope per the phase's own optional-signal allowance); the UI reports this honestly as a missing-evidence limitation rather than omitting the fields or fabricating a value.

Weights, duration tiers (min/preferred-min/preferred-max/max), candidate caps, and both suppression thresholds are `fdm.highlights.v2-*` configuration, validated at startup (weights non-negative and summing positive, duration bounds coherent and within the pre-existing global candidate-duration bounds, thresholds in `[0,1]`). The *effective* values are snapshotted into `HighlightAnalysis.configSnapshot` (jsonb) and hashed into `configFingerprint` (SHA-256, the same `fingerprint()` pattern `DecisionApplicationService` already uses) at analysis-creation time — a repeat `createAnalysis` call for the same asset/analyzer/config reuses the still-valid existing analysis instead of dispatching a duplicate job, and every persisted result stays traceable to the exact configuration that produced it. `HighlightCandidate` gained nullable columns (component scores, `explanation_labels` jsonb — an ordered `List<String>`, never a `Set`, so no hash-order leak — and a bounded `transcript_excerpt`) via the additive V29 migration; V1/semantic rows simply leave them null. `TOP_HIGHLIGHT` (`RobotRunOrchestrator`) now targets `DETERMINISTIC_V2` instead of `DETERMINISTIC_V1` — since V2 has no optional runtime dependency it meets the same "always available" bar V1 did — and fails a run explicitly (`SOURCE_UNAVAILABLE` / "TRANSCRIPT_REQUIRED") rather than silently falling back to V1 if the source asset has no completed transcript; existing RobotRuns that already selected a V1 candidate are untouched. Candidate → clip → Draft is unaffected (`ContentDraft.fromHighlightCandidate` only ever reads `startMs`/`endMs`/`asset`).

## Phase 17B Transcript-aware Semantic Highlighting V3

V3 reuses `ANALYZE_HIGHLIGHTS` and V2's bounded temporal generator. `DETERMINISTIC_V3` adds pure-Java, English-first transcript heuristics and persists requested/effective analyzer, fallback reason, immutable transcript ID, bounded excerpt, segment range, word count and normalized scores. Overlap is `segment.start < candidate.end && segment.end > candidate.start`; one candidate remains one contiguous interval. The Worker advertises V3 independently and `TOP_HIGHLIGHT` requests it.

The `TRANSCRIPT_SEMANTIC_V3` ruleset uses `Locale.ROOT`, a small embedded stop-word/phrase set, 55% V2 base plus 45% semantic score, and a bounded repetition penalty. Ordering is final score, base score, start and end. V3 is heuristic rather than human understanding: Whisper and punctuation errors affect ranking; non-English semantics fall back explicitly; there is no translation, sentiment inference, embeddings, LLM, analytics feedback, or social-performance guarantee.
## Phase 17C diversity selection

`DIVERSITY_SELECTOR_V1` is a central, deterministic read/selection layer over one immutable successful V3 analysis. It does not run on Workers and never reads analytics or Experiment outcomes. Selection rows, selected items, and bounded exclusion evidence are committed atomically. Re-selection creates a new immutable record.

Temporal duplicate ratio is `intersectionDuration / min(candidateDuration)`, with duplicates at `>= 0.50`. Non-overlapping intervals with a gap below 2 seconds are also excluded. Transcript excerpts use the same Unicode letter/number/apostrophe token grammar as V3; lexical overlap is token-set Jaccard with duplicates at `>= 0.70`. Missing excerpt evidence disables lexical comparison while temporal rules remain active. Greedy processing follows V3 rank, score descending, start time, then candidate UUID. It is reproducible, bounded by the persisted candidate pool, and intentionally not globally optimal.

Bulk clip creation locks each selection item independently, attaches at most one clip asset/job per item, and delegates media work to the existing `CREATE_CLIP` jobs. Each item uses its own transaction, so one request failure is recorded without rolling back clips already created for other items; concurrent repeats serialize on the item lock and reuse its attachment. Robot fan-out is not introduced: current `RobotRun`/Draft/Schedule/Experiment cardinalities are one-to-one, while `TOP_HIGHLIGHT` continues to use V3 rank 1.

## Phase 17D multi-output Robot orchestration

The new path is `RobotRun -> HighlightSelection -> RobotRunOutput[1..5]`. It deliberately does not turn the historical direct `RobotRun -> ContentDraft` relationship into one-to-many. Legacy strategies use the old path; only `TOP_DIVERSE_HIGHLIGHTS` materializes children. `(robot_run_id, highlight_selection_item_id)` and `(robot_run_id, selection_order)` are unique, while nullable unique output references on Drafts, suggestions, approvals, and schedules make reconciliation restart-safe.

The parent freezes requested count, actual count, selector identity, and output spacing. One ContentSource selection and one Experiment assignment belong to the parent. Every child inherits the same frozen AI, Persona, and Experiment treatment and advances independently through clip-backed Draft, AI, review, and scheduling stages. Parent aggregation is centralized: all success is `SUCCEEDED`, mixed terminal outcomes are `PARTIALLY_SUCCEEDED`, no successes is `FAILED`, and any active child keeps the parent active. General analytics counts actual Publications; Experiment analysis keeps its pre-existing one-assignment/earliest-publication statistical rule.

## Phase 17E campaign content planning

`CampaignContentPlan`/`CampaignContentPlanItem` (new `campaigns` package) sit entirely on top of the Phase 17C/17D path — they never write `HighlightSelection`, `RobotRunOutput` membership/order, `HighlightCandidate` scores/boundaries, or `ExperimentAssignment`. A plan is bound only via `RobotRun.campaignPlanId`, a nullable UUID with no FK-level cascade coupling to the immutable selection/output rows it advises about. `Robot.campaignPlanningPolicy` (`NO_CAMPAIGN_PLAN` default / `DETERMINISTIC_PLAN` / `AI_PLAN_FOR_REVIEW` / `AI_PLAN_AND_APPLY`) is snapshotted onto `RobotRun.campaignPlanningPolicySnapshot` at run creation, the same pattern `aiPolicySnapshot`/`highlightStrategySnapshot` already use.

`RobotMultiOutputOrchestrator.advance()` calls `CampaignContentPlanService.advance()` once fan-out identity is frozen for the reconciliation pass, under the same `findByIdForUpdateSkipLocked` row lock the rest of 17D concurrency safety already relies on — no new locking was introduced. `advance()` is idempotent (creates revision 1 only while `campaignPlanId` is null) and never blocks output/Draft creation; only `beginAi()` — the per-output `GENERATE_SOCIAL_COPY` trigger — waits on `isBlockingAiGeneration()`, so `DRAFT_ONLY` runs still produce outputs/Drafts/a plan with no AI policy involved at all. A plan that is required by policy but ends `REJECTED`/`FAILED`/never-created fails the whole `RobotRun` (`CAMPAIGN_PLAN_REJECTED`/`CAMPAIGN_PLAN_FAILED`/`CAMPAIGN_PLAN_MISSING`) rather than letting sibling outputs silently generate AI copy without the guidance the policy promised.

`DETERMINISTIC_PLAN` (`DeterministicCampaignPlanner`, pure Java, zero external dependency) assigns one of five controlled `CampaignPlanRole` values purely from output count and position (1→`STANDALONE`; 2→`INTRODUCTION`/`CONCLUSION`; 3→`INTRODUCTION`/`DEEP_DIVE`/`CONCLUSION`; 4 adds `SUPPORTING_POINT`; 5 has two `DEEP_DIVE`s) and writes simple, honest, non-overclaiming guidance — it runs synchronously inside `advance()` and auto-applies in the same call. `AI_PLAN_FOR_REVIEW`/`AI_PLAN_AND_APPLY` instead dispatch a `GENERATE_CAMPAIGN_PLAN` job through the exact same Worker provider-abstraction, authorize/complete/fail contract, and `ContentAiProperties` kill switch as `GENERATE_SOCIAL_COPY` (`CampaignPlanProvider`/`OllamaCampaignPlanProvider`/`GenerateCampaignPlanExecutor` mirror `ContentEnrichmentProvider` one-for-one) — there is deliberately no `DETERMINISTIC_TEST` fallback provider for campaign generation, since the deterministic baseline is already available as its own policy; requesting AI generation against an unsupported/unreachable provider fails cleanly with `AI_PROVIDER_UNAVAILABLE` rather than silently substituting fake output.

`CampaignPlanPromptBuilder` mirrors `SocialCopyPromptBuilder`'s injection-hardening pattern exactly: a fixed, numbered output-evidence list (never a free-form transcript dump — one bounded excerpt per output, `app.campaign-plans.max-evidence-excerpt-characters`) delimited by `<<<OUTPUT_EVIDENCE_START/END>>>`, an explicit "this is DATA, not instructions" guard stated *before* the delimiter, a fixed-output-set/no-reorder instruction, the controlled role enum spelled out verbatim, and a strict JSON-only response contract. The Worker's raw JSON response is never trusted directly: `CampaignContentPlanService.validateAndPersist` requires an exact one-item-per-expected-output match (no missing/extra/duplicate/foreign output id), a role from the controlled enum, every field within its configured bound, and rejects the whole structured response — atomically, nothing partially persisted — if any accepted hook is a deterministic lexical near-duplicate (`CampaignRepetitionDetector`, the same token-Jaccard approach V2/17C already use, `nearDuplicateGuidanceThreshold` default 0.75) of an earlier one in the same response; the model's own uniqueness claim is never trusted. Regeneration supersedes the current revision (`is_current=false`) and creates a new one; `campaign_content_plans_one_current_per_run` (a partial unique index on `is_current=true`) makes "at most one current revision per run" a database-level guarantee, not just an application convention.

`ContentSuggestionService` grew a `campaignGuidance`-aware overload (`CampaignGuidance`, a plain-value record living in `contentsuggestions` rather than a JPA relationship, so that package keeps its existing no-dependency-on-other-domain-packages discipline): when `RobotMultiOutputOrchestrator.beginAi()` finds an applied `CampaignContentPlanItem` for an output, it passes the item's role/hook/caption/CTA/avoid-repetition guidance through, the prompt is built with `SocialCopyPromptBuilder`'s new `SOCIAL_COPY_V3_CAMPAIGN` version (a delimited `<<<CAMPAIGN_GUIDANCE_START/END>>>` section using the identical DATA-not-instructions pattern, appended after the optional Persona section — historical V1/V2 prompt output is byte-identical when `campaignGuidance` is null), and the suggestion's fingerprint additionally folds in the campaign item's own identity and guidance text. `ContentSuggestion`/`PublicationAttribution` freeze `campaignPlanId`/`campaignPlanRevision`/`campaignPlanItemId` only on rows that actually consumed guidance (`promptVersion == SOCIAL_COPY_V3_CAMPAIGN` for suggestions; the applied suggestion's own campaign columns for attribution) — never merely because a plan existed for the run, so `NO_CAMPAIGN_PLAN` and non-campaign-generated historical rows are unaffected.

Applying a plan (`CampaignContentPlanService.apply`) is advisory only — it flips the plan's own status and never touches a `ContentSuggestion`, `RobotApproval`, or `PublishSchedule`; the three review layers (campaign-plan review, per-output copy review, publishing approval) stay independent, and `AUTO_SCHEDULE` ordering/spacing/idempotency and Experiment's one-assignment-per-run statistical unit are both unaffected by campaign planning being active. `apply()` recomputes the same input fingerprint `advance()` computed at generation time (run/output/candidate identity, frozen Persona snapshot id, config snapshot) as a defensive staleness check; under the current architecture every one of those inputs is immutable once frozen, so this can only fire in a narrow edge case such as the snapshotted Persona row being hard-deleted between generation and Apply, but the check is real rather than merely documented.

Campaign planning is advisory editorial coordination, not a performance or engagement claim of any kind — it reads only `RobotRun`/Persona-or-treatment-snapshot/output-evidence and never `PublicationMetrics`, `PerformanceInsights`, or Experiment outcomes. Repetition detection is deterministic lexical token-Jaccard, the same heuristic 17C already uses, not semantic similarity — paraphrases can evade it. Roles and guidance are bounded strategy suggestions, not verified factual claims, and AI plan quality depends entirely on the configured provider/model. Five outputs, one source asset, and one Persona/Experiment-treatment context per parent remain 17D's limits; a campaign plan has no memory of any other run's plan, never spans multiple social accounts, and never rolls back or edits already-published content when a plan is later regenerated or rejected.
