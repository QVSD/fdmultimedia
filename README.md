...
`DETERMINISTIC_V2` ("Semantic Highlighting V2") ranks highlight candidates from already-persisted transcript segments using deterministic feature scoring — no LLM, no embeddings, fully offline. It generates bounded candidate windows from Unicode-aware sentence boundaries, scores each on hook strength, completeness, information density, a preferred speech-density band, boundary alignment, and transcript coverage, then applies deterministic non-maximum suppression (temporal overlap + text-similarity duplicate suppression) with stable tie-breaking. Every score is decomposable and every candidate carries deterministic explanation labels (e.g. `CLEAN_OPENING`, `HIGH_SPEECH_DENSITY`) and a bounded transcript excerpt. Effective configuration is fingerprinted (SHA-256) and snapshotted onto the analysis, so an identical repeat request reuses the same result instead of duplicating work, and the exact config that produced a result stays auditable. V1 (`DETERMINISTIC_V1`) and the Ollama-backed `TRANSCRIPT_SEMANTIC_V1` are untouched and remain independently selectable; Robots' `TOP_HIGHLIGHT` now targets V2 (no optional-dependency risk, unlike the Ollama analyzer) and fails a run explicitly if the source asset has no completed transcript, rather than silently falling back to V1. Scene/audio signals are optional future enrichment and are honestly reported as unavailable in this phase — no fabricated evidence.

# Phase 17B: Transcript-aware Semantic Highlighting V3

`DETERMINISTIC_V3` deterministically reranks V2's bounded windows using persisted English transcript signals: lexical and speech density, hook/question phrases, emphasis/contrast, self-containedness, repetition and boundary quality. The fixed formula retains the V2 base at 55% and semantic heuristics at 45%, with a bounded repetition penalty. It never calls an LLM, embeddings, analytics, or an external service. Analyses retain the exact transcript and bounded evidence; unusable input records requested/effective analyzers and a controlled fallback reason. Scores are heuristics, not probabilities or performance predictions.
### Phase 17C: diverse highlight selections

Successful `DETERMINISTIC_V3` analyses can be turned into immutable, auditable shortlists of 1–5 moments with `DIVERSITY_SELECTOR_V1`. The central API walks the persisted V3 rank order and rejects candidates that overlap at least 50% of the shorter interval, are less than 2 seconds from an already selected interval, or have at least 70% normalized token-set Jaccard overlap. Original V3 scores, ranks, evidence, and boundaries are never changed.

Selections may be `COMPLETE`, `PARTIAL`, or `EMPTY`; fewer results than requested is a valid diversity outcome. Exclusions retain the conflicting candidate and measured overlap where applicable. Bulk clip creation reuses the existing `CREATE_CLIP` pipeline and is idempotent per selection item. Vertical creation remains deliberate per clip. `TOP_HIGHLIGHT` remains a one-candidate Robot workflow; multi-output Robot fan-out is deferred because one `RobotRun` currently owns exactly one Draft, Schedule, Publication attribution chain, and Experiment assignment.
